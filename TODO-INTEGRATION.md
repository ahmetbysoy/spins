...
## BACKLOG
- ~~**Sağ şerit overlay katmanı** (madde 4)~~ → `b184dee` göçüyle ÇÖZÜLDÜ: `chart-wrap` artık canvas-only; ⚡/🐋/👻/🌊/🛡/💥 rozetleri chart'ın yanındaki `w-12` DOM sütuna, DUVAR tekeri alt banda (h-7 `fs-marquee`), LİKİDİTE akışı üst banda taşındı. placeLabelY koordinatörüne artık gerek yok (etiket canvas dışında).
