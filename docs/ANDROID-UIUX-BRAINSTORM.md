...
> **Uygulandı (2026-09-02):** ek: dokunmatik crosshair — uzun basma inceleme (magnet snap + sürükleyerek gez + haptic tick + pan kilidi), crosshair mode dokunmatikte Magnet; P0/1 geri tuşu (`lib/back-button.ts` LIFO + `use-android-back`, 4 bağlantı), P0/2 touch-action/tap-highlight/user-select, P0/3 16px input zoom, P0/4 `interactive-widget=resizes-content` (100dvh zaten vardı), P0/5 tam ekran ↙ çıkış butonu; P1/10 PWA manifest + maskable ikon; P2/12 visibilitychange canvas/poll kilidi. Kalan: hover→active, 44px denetimi, haptics, snap scroll, ağ farkındalığı.
